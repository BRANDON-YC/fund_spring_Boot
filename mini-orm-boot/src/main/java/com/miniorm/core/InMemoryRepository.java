...
public class InMemoryRepository<ID, T> implements GenericRepository<ID, T> {
    ...
    public T save(T entity) {
        ID id = getId(entity);
        if (id == null || (id instanceof Number n && n.longValue()==0L)) {
            id = generateId();
            setId(entity, id);
        }
    ...
    @Override public Optional<T> findById(ID id) { return Optional.ofNullable(storage.get(id)); }
    @Override public java.util.List<T> findAll() { return new java.util.ArrayList<>(storage.values()); }

    @Override public T update(ID id, T entity) {
    ...
    @Override public boolean deleteById(ID id) { return storage.remove(id) != null; }
}